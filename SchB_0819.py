...
def train(args):
    ...
    f_theta = velocity_net(d=args.d)
    f_theta = f_theta.to(device)
    optim = Adam(f_theta.parameters(), lr=args.lr)


    adjust = 1* args.batch_size * args.model_ema_steps / args.epochs
    alpha = 1.0 - 0.995
    alpha = min(1.0, alpha * adjust)
    model_ema = ExponentialMovingAverage(f_theta, device=device, decay=1.0 - alpha)
    ...
    for epoch in tqdm_epoch:
        ...
        z_terminal=z_temp
        log_terminal=log_Gaussian(x=z_terminal,d=args.d)
        ...
        z,log_temp,bridge_pen=cat_var[:,0:args.d],cat_var[:,args.d],-cat_var[0,-1]
        optim.zero_grad()
        ...
        if epoch%args.model_ema_steps==0:
            model_ema.update_parameters(f_theta)
        ...
        torch.save(f_theta.state_dict(), 'experiment/'+args.dataset+'_2d.pth')
        # if (epoch+1) % 200 == 0:
